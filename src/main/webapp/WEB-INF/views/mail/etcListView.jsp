							...
							<div class="mail-main-form-list">         
								<c:choose>
									...
									<c:when test="${ boxType eq 7 }">
										<span class="mail-h">안읽은 메일함</span>
									</c:when>
								</c:choose>
										...
										<table class="mail-table-list">
											...
											<c:choose>
												<c:when test="${ empty list }">
													<tr>
														<td colspan="5" class="noList">현재 메일이 없습니다.</td>
													</tr>
												</c:when>
												<c:otherwise>
										...
										<script>
											...
											$(function(){
												...
												$(document).on("click", "ion-icon[name=copy-outline]", function(){
													...
													let url = 'detail.ma?no=' + $(this).parent().siblings().eq(0).children().val() + '&boxType=' + ${ boxType } + '&preview=2'; 
